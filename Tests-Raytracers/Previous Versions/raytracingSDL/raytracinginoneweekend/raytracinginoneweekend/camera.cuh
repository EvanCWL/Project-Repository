...
class camera {
public:
	__device__ camera(vec3 lookfrom, vec3 lookat, vec3 vup, double vfov, double aspect, double aperture, double focus_dist) { // vfov is top to bottom in degrees
		lens_radius = aperture / 2.0f;
		double theta = vfov * ((double)M_PI) / 180.0f;
