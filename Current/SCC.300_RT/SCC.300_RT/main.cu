...
__global__ void create_world(hitable** d_list, hitable** d_world, camera** d_camera, int width, int height, curandState* rand_state, int xc) {
	if (threadIdx.x == 0 && blockIdx.x == 0) {
...
int main(int argc, char* argv[]) {
	...
	while (running) {
		...
		while (SDL_PollEvent(&event)) {
			...
			break;
			case SDL_KEYDOWN: 					// ... Handle other events here. **IMPLEMENTATION OF MOUSE DETECTIONS**
				switch (event.key.keysym.sym) {
					case SDLK_LEFT: xc--; std::cout << xc << "\n"; break;
					case SDLK_RIGHT: xc++; std::cout << xc << "\n"; break;
					case SDLK_RETURN:
						clock_t start, stop;
						start = clock();
		...
		SDL_SetRenderDrawColor(renderer, fb[pixel_index].r(), fb[pixel_index].g(), fb[pixel_index].b(), 255);
		SDL_Rect rectangle;
		rectangle.x = i;
