...
public:
	...
	__device__ virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered, curandState* local_rand_state) const {
		vec3 target = rec.p + rec.normal + Math::random_in_unit_sphere(local_rand_state);
...
public:
	...
	__device__ virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered, curandState* local_rand_state) const {
		...
		scattered = ray(rec.p, reflected + fuzz * Math::random_in_unit_sphere(local_rand_state));
		attenuation = albedo;
